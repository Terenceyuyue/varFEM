...
for k = 1:maxIt
    ...
    uh = FEM1D_variational(Th,pde,Vh,quadOrder); 
    nel = size(elem1D,1);
    h(k) = 1/nel;
    ErrL2(k) = getL2error1D(node,elem1D,uh,pde.uexact,Vh,quadOrder);
    ErrH1(k) = getH1error1D(node,elem1D,uh,pde.Du,Vh,quadOrder);
end
