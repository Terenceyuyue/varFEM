...
VhNew = repmat( {Vh}, 1, 2 ); % v = [v1,v2]
for k = 1:maxIt
    [node,elem] = uniformrefine(node,elem);
    bdStruct = setboundary(node,elem,bdNeumann);
    Th = getTh(node,elem); 
    uh = elasticity2_variational(Th,pde,VhNew,quadOrder);
    uh = reshape(uh,[],2);
    ...
    for id = 1:2
        ...
        errL2(:,id) = getL2error(node,elem,uid,u,Vh,quadOrder);
        errH1(:,id) = getH1error(node,elem,uid,Du,Vh,quadOrder);
    end
